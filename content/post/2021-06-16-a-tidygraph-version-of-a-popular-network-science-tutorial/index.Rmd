...
subtitle: ''
summary: 'A reworking of @Ognyanova  tutorial using tidygraph'
authors: []
...
This is an attempt to rework , using `tidygraph`  and `ggraph`, much of [**Network Visualization with R**](www.kateto.net/polnet2018) Polnet 2018 Workshop Tutorial, Washington, DC by Katherine (Katya) Ognyanova.
...
graph %>% 
  ...
  geom_edge_link(color = "grey",
                 ...
                 arrow = arrow(type = "closed", 
                               ends = "both", 
                               length = unit(1, "mm"))) + 
  geom_node_point(fill = "orange", shape = 21, size = 6, color = "#555555") + 
...
links %>% 
  ...
  geom_node_point(aes(fill = type.label,
                      # type.label is available as edge attribute
                      size = size),
                  shape = 21,
                  color = "black") +
  scale_fill_manual(
    ...
    values = c("grey50", "gold", "tomato"),
    guide = "legend") +
  scale_edge_color_manual(name = "Source Type", values = c("grey80", "gold", "tomato"))+ 
  scale_edge_width(range = c(0.2, 1.5), guide = "none") +
  scale_size_continuous("Degree", range =  c(2, 16)) +
  # not "limits"!
...
proj1 <- as_incidence_matrix(net2)  %*% t(as_incidence_matrix(net2)) %>% 
  as_tbl_graph() %>% 
  activate(nodes) %>% 
  left_join(., nodes2, by = c("name" = "id"))
proj2 <- t(as_incidence_matrix(net2)) %*%   as_incidence_matrix(net2) %>% as_tbl_graph() %>% 
  activate(nodes) %>% 
  left_join(., nodes2, by = c("name" = "id"))

#overlay = igraph::layout_with_graphopt(net2)
...
p2 <- proj2 %>% 
  ggraph(layout = "graphopt") + 
  geom_edge_link0() + 
  geom_node_point(aes(colour = media.type), size = 6, shape  =15, colour = "dodgerblue") + 
  geom_node_text(aes(label = media), repel = TRUE)
...
See the edges between `s03` and `s10`...these are in opposite directions. 
